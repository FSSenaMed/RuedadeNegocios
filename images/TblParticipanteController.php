	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','Seleccion', 'Intermediary', 'Info', 'Inscripcion'),
				'users'=>array('*'),
			),
	...
	public function actionCreate($nit)
	{
		...
		{	
			...
			if(isset($_FILES) and $_FILES['TblParticipante']['error']['foto']==0){
				...
				$nombre=$model->parti_nombreempresa;
				$Urlimage="/images/".$nombre.'.'.$extencion;

				$uf->saveAs(Yii::getPathOfAlias('webroot').'/images/'.$nombre.'.'.$extencion);
